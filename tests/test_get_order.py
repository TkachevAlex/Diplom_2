...
class TestGetOrder:
    ...
    def test__get_order__authorized_user__order_is_getting(self, create_user):
        user = create_user
        order = Order()
    ...
    @allure.title(f'При запросе заказов неавторизованного пользователя, в ответе возвращается ошибка 401: "{Error.AUTHORISED}"')
