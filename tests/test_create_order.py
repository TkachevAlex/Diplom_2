...
class TestCreateOrder:
    ...
    def test__create_order__authorized_user_and_ingredients__status_code_is_200(self, create_user):
        user = create_user
    ...
    @allure.title(f'При передаче в ручку создания заказа токена юзера, в ответе возвращается ошибка 400: "{Error.NON_INGREDIENT}"')
    def test__create_order__authorized_user_and_none_ingredients__must_be_provided(self, create_user):
        user = create_user
        ingredients = []
        order = Order()
        response = order.create_order(ingredients, user)
        assert (response.status_code == 400) and (loads(response.text)['message'] == Error.NON_INGREDIENT)
    ...
    @allure.title(f'При передаче в ручку создания заказа списка без ингредиентов, в ответе возвращается ошибка 400: "{Error.NON_INGREDIENT}"')
    ...
    def test__create_order__authorized_user_and_bad_hash__status_code_is_500(self, create_user):
        user = create_user
        ingredients = IngredientData().generate_data()
